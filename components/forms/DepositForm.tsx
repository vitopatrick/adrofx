...
import { walletOne, walletTwo } from "@/lib/wallet-address";
...
const Form = () => {
  ...
  const [switchCoin, setCoin] = useState<any>(
    user?.wallet == 1 ? walletOne : walletTwo
  );
  ...
  useEffect(() => {
    ...
    const subscription = watch((value) => {
      const selectedCoin = switchCoin.find(
        (address: any) => address.sym === value.coin
      );
